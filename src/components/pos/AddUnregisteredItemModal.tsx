...
export const AddUnregisteredItemModal: React.FC<Props> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
      <div
        className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border animate-in fade-in zoom-in-95"
        style={{ borderColor: branchTheme.colors.cardBorder }}
      >
        {/* Header */}
        <div className="px-4 sm:px-5 py-3 border-b border-gray-200 flex items-center justify-between bg-[#FBFAF6] shrink-0">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ backgroundColor: branchTheme.colors.primaryLight, color: branchTheme.colors.primary }}
            >
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-3.5 overflow-y-auto flex-1 text-xs">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-[#FBFAF6] border border-gray-200 rounded-xl text-xs font-semibold text-[#111111] focus:outline-none focus:border-[var(--pos-primary)] focus:bg-white transition-colors"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full px-3 py-2 bg-[#FBFAF6] border border-gray-200 rounded-xl text-xs font-bold text-[#111111] focus:outline-none focus:border-[var(--pos-primary)] focus:bg-white transition-colors"
              />
            </div>
            <div>
              <label className="block font-bold text-[#374151] mb-1">
          ...
          <div className="p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-[11px] text-gray-700 flex items-start gap-2">
            <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" style={{ color: branchTheme.colors.primary }} />
          ...
          <div className="pt-2 flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="h-8 px-3.5 text-[11px] font-bold rounded-lg border hover:brightness-110 transition-all shadow-xs disabled:opacity-50 flex items-center gap-1.5 cursor-pointer"
            >
              {isSubmitting ? (
                <>
                  <span
                    className="w-3 h-3 border-2 rounded-full animate-spin inline-block"
                    style={{ borderColor: `${branchTheme.colors.primary}33`, borderTopColor: branchTheme.colors.primary }}
                  />
                  <span>{l('Adding...', 'சேர்க்கிறது...')}</span>
                </>
              ) : (
                <>
                  <PlusCircle className="w-3.5 h-3.5" style={{ color: branchTheme.colors.primary }} />
